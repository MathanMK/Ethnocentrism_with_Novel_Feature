...
public class Grid {
    ...
    public boolean isWithinRRA(Coord coord) {
        if (rra == null) {
            return false;
        }
        ...
        int rraRow = rra.getRow();
        int rraCol = rra.getCol();
        int radius = rra.getRadius();

        int distance = Math.max(Math.abs(row - rraRow), Math.abs(col - rraCol));
