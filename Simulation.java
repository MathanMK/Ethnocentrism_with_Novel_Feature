...
public class Simulation {
    ...
    private int rraRadius ; //change the radius of the rra
    ...
    public class RunStats {
        ...
        int maxPycor;
        int step;
        double sumCoopownPercent, sumSqCoopownPercent;
        double sumDefotherPercent, sumSqDefotherPercent;
        double sumConsistEthnoPercent, sumSqConsistEthnoPercent;
        double sumMeetownPercent, sumSqMeetownPercent;
        double sumCoopPercent, sumSqCoopPercent;
        double sumLast100coopownPercent, sumSqLast100coopownPercent;
        double sumLast100defotherPercent, sumSqLast100defotherPercent;
        double sumLast100consistEthnoPercent, sumSqLast100consistEthnoPercent;
        double sumLast100meetownPercent, sumSqLast100meetownPercent;
        double sumLast100coopPercent, sumSqLast100coopPercent;
        double sumCcPercent, sumSqCcPercent;
        double sumCdPercent, sumSqCdPercent;
        double sumDcPercent, sumSqDcPercent;
        double sumDdPercent, sumSqDdPercent;

        int countCC, countCD, countDC, countDD;
    }
    ...
    private void setupFull() {
        clearAll();
        initializeVariables();
      //Create and place the RRA
        int rraRow = numRows / 2; // Place the RRA at the center
        int rraCol = numCols / 2;
        RRA rra = new RRA(rraRow, rraCol, rraRadius);
        grid.setRRA(rra);
 ...
 // use PTR to determine if the agent gets to reproduce
    private void reproduce(Agent agent, Coord coord) {
        ...
        double reproductionRate = agent.getPtr();

        // Check if the agent is within the RRA
        if (grid.isWithinRRA(coord)) {
            // Double the reproduction rate within the RRA
            reproductionRate *= 2.0;
        }

        if (rand.nextDouble() < reproductionRate) {
    ...
    private RunStats getRunStats() {
        ...
        rs.sumCoopownPercent = coopownPercent();
        rs.sumSqCoopownPercent = coopownPercent() * coopownPercent();
        rs.sumDefotherPercent = defotherPercent();
        rs.sumSqDefotherPercent = defotherPercent() * defotherPercent();
        rs.sumConsistEthnoPercent = consistEthnoPercent();
        rs.sumSqConsistEthnoPercent = consistEthnoPercent() * consistEthnoPercent();
        rs.sumMeetownPercent = meetownPercent();
        rs.sumSqMeetownPercent = meetownPercent() * meetownPercent();
        rs.sumCoopPercent = coopPercent();
        rs.sumSqCoopPercent = coopPercent() * coopPercent();
        rs.sumLast100coopownPercent = last100coopownPercent();
        rs.sumSqLast100coopownPercent = last100coopownPercent() * last100coopownPercent();
        rs.sumLast100defotherPercent = last100defotherPercent();
        rs.sumSqLast100defotherPercent = last100defotherPercent() * last100defotherPercent();
        rs.sumLast100consistEthnoPercent = last100consistEthnoPercent();
        rs.sumSqLast100consistEthnoPercent = last100consistEthnoPercent() * last100consistEthnoPercent();
        rs.sumLast100meetownPercent = last100meetownPercent();
        rs.sumSqLast100meetownPercent = last100meetownPercent() * last100meetownPercent();
        rs.sumLast100coopPercent = last100coopPercent();
        rs.sumSqLast100coopPercent = last100coopPercent() * last100coopPercent();
        rs.sumCcPercent = ccPercent();
        rs.sumSqCcPercent = ccPercent() * ccPercent();
        rs.sumCdPercent = cdPercent();
        rs.sumSqCdPercent = cdPercent() * cdPercent();
        rs.sumDcPercent = dcPercent();
        rs.sumSqDcPercent = dcPercent() * dcPercent();
        rs.sumDdPercent = ddPercent();
        rs.sumSqDdPercent = ddPercent() * ddPercent();
        rs.countCC = countAgentsWith(true, true);
        rs.countCD = countAgentsWith(true, false);
        rs.countDC = countAgentsWith(false, true);
        rs.countDD = countAgentsWith(false, false);
